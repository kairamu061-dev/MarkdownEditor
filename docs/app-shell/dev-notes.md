...
| アイコンを差し替えても exe のアイコンだけ古いまま（2026-08-04 実機報告 / BUG-024） | `tauri-build` は `icons/icon.ico` を exe のリソース 32512 に埋め込むが、`cargo:rerun-if-changed` をアイコンに対して出していない（`tauri-build-2.6.3` 内を全数確認）。そのためアイコンだけ差し替えても `build.rs` が再実行されず、古いリソースがリンクされ続ける。タスクバー側は別経路（`generate_context!` の埋め込み資産）のため更新され、この非対称な症状になる。`build.rs` に `println!("cargo:rerun-if-changed=icons");` を追加して解決 |
