...
| アプリアイコンに羽根モチーフを採用（2026-08-04） | 羽根ペン＝執筆のメタファがエディタの用途に合う。当初は角丸ダークプレート付きの案（背景 `#2E3643`・羽根 `#88C2CD`、Nord の nord0 / nord8 とほぼ一致）を採用したが、同日中に曲線的な `curved_feather_simple_1` へ差し替え、プレートを廃して背景透過とした（ユーザー判断） |
| 差し替え後の配色は Nord パレット外（2026-08-04） | 現行アイコンの実測値は輪郭 `#49ABC1` / 本体 `#9CD6E0` / ハイライト `#CDEEF2`。nord8 `#88C0D0` より彩度・明度とも高く、同系統のシアンではあるが Nord の値そのものではない。初代案にあった「アイコンと UI が同一パレット」という根拠は現行アイコンには当てはまらない |
| アイコン原画（1024px）を `assets/icon-source-1024.png` として追跡下に置く | 生成元は `tmp/` にあったが同ディレクトリは .gitignore 対象で、そのままではアイコンセットを再生成できなくなるため。`tauri icon` はこの追跡パスから実行する。原画は**透過処理済みのものを保存**する（下記コマンドの出力） |
| 背景の白抜きはグローバル置換ではなく外周からの塗りつぶしで行う | `-transparent white` のような全画素置換だと、羽根の内側にある白いハイライト（軸に沿った線）まで穴が開く。外周に連結した白だけを消す floodfill を使い、閉じた内部の白は残す。fuzz はアート中の最も淡い色 `#CDEEF2`（白から約 29% 差）を消さないよう 10% に留める。<br>`convert curved_feather_simple_1.png -alpha set -bordercolor white -border 1 -fuzz 10% -fill none -floodfill +0+0 white -shave 1x1 out.png` |
...
| アイコンを差し替えても exe のアイコンだけ古いまま（2026-08-04 実機報告 / BUG-024） | 主因は `tauri icon` が出力する `icon.ico` の全エントリが PNG 圧縮だったこと。Windows が ICO の PNG 圧縮エントリに対応するのは 256px のみで、16/24/32/48/64 は BMP/DIB でないと描画できない。exe の PE リソースを解析して形式まで確認して判明した。ICO のみ ImageMagick で作り直して解決（`-define icon:auto-resize=256,64,48,32,24,16`）。副因として `tauri-build` がアイコンに `cargo:rerun-if-changed` を出さない件もあり、`build.rs` にディレクトリ監視を追加した |
| `tauri icon` を素で叩くと上記が黙って再発する | `scripts/gen-icons.sh` に生成手順（PNG 生成 → モバイル用削除 → ICO 再生成 → 形式検証）をまとめ、256 未満に PNG エントリが残っていたら非ゼロ終了するようにした。アイコンを差し替えるときはこのスクリプトを使う |
| Linux 側で `tauri icon` を実行しようとしたら native binding 不在で失敗（2026-08-04） | BUG-023 の対処でユーザーが Windows 側 `npm ci` を実行し、共有ツリーの `node_modules` が Windows 用に入れ替わったため（`cli-win32-x64-msvc` のみ・`.cmd` シム 7 個を実測）。BUG-023 で記録した「共有ツリーでは両 OS を同時に満たせない」制約が実際に顕在化した形。Windows 側のビルド環境を壊さないよう、プロジェクトの `node_modules` には触れず `npm i --prefix /tmp/tauricli @tauri-apps/cli@2` で別prefixに CLI を入れて `-o src-tauri/icons` 指定で生成した |
...
- アイコン 16px（ICO 内の最小サイズ）では羽枝のディテールが潰れて輪郭のぼやけた塊になる。角丸プレートを廃したことで濃い輪郭という手掛かりが無くなり、初代案よりも 16px の判別性は落ちている。特に白背景（エクスプローラのライトモード等）では淡い本体色が背景に沈む。32px は良好、48px 以上は明瞭。改善するなら 16/32px 用にコントラストを上げた専用ビットマップを ICO に差し込む（マルチ解像度 ICO の常套手段）
- 背景透過・淡いシアンのため、白背景上でのコントラストはダーク背景上より弱い。プレート付き案とのトレードオフとして受容している
- 作業ツリーの実体は Windows の `D:\AI\Antigravity\MarkdownEditor` で、devcontainer にマウントされている（`target/release/build/*/out/resource.rc` に埋め込まれた絶対パスから判明、2026-08-04）。`src-tauri/target/` も共有されるため、Windows 側のビルド成果物を devcontainer から直接検査できる。BUG-024 の切り分けはこれを利用した
